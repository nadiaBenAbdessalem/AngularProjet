...
var Popover = Class.extend({
	...
	// Triggered when the user.ts clicks *anywhere* in the document, for the autoHide feature
...
var DragListener = Class.extend({
	...
	// Call this when the user.ts does a mousedown. Will probably lead to startListening
	...
	// Called when the user.ts moves the mouse
	...
	// Called when the user.ts does a mouseup
	mouseup: function(ev) {
		this.stopDrag(ev);
	...
	// Call this to stop listening to the user.ts's mouse events
...
var MouseFollower = Class.extend({
	...
	// Gets called when the user.ts moves the mouse
	mousemove: function(ev) {
...
Grid.mixin({
	...
	mousedOverSeg: null, // the segment object the user.ts's mouse is over. null if over nothing
	...
	destroyEvents: function() {
		this.triggerSegMouseout(); // trigger an eventMouseout if user.ts's mouse is over an event
	...
	// Called when the user.ts does a mousedown on an event, which might lead to dragging.
	...
	// Called when the user.ts does a mousedown on an event's resizer, which might lead to resizing.
	// Generic enough to work with any type of Grid.
...
DayGrid.mixin({
	...
	showSegPopover: function(cell, moreLink, segs) {
		...
		options = {
			...
			autoHide: true, // when the user.ts clicks elsewhere, hide the popover
...
var View = fc.View = Class.extend({
	...
	isSelected: false, // boolean whether a range of time is user.ts-selected or not
	...
	// Computes the new date when the user.ts hits the prev button, given the current date
	...
	// Computes the new date when the user.ts hits the next button, given the current date
	...
	// Sets the scroll value of the scroller to the initial pre-configured state prior to allowing the user.ts to change it
	...
	// Computes if the given event is allowed to be dragged by the user.ts
	...
	// Computes if the given event is allowed to be resize by the user.ts
	...
	// Handler for unselecting when the user.ts clicks something and the 'unselectAuto' setting is on
	documentMousedown: function(ev) {
		var ignore;

		// is there a selection, and has the user.ts made a proper left click?
	...
	function refetchEvents() { // can be called as an API method
		destroyEvents(); // so that events are cleared before user.ts starts waiting for AJAX
